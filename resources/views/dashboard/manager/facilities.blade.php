...
@section('dashboard-content')
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-bold text-slate-900">Kelola Fasilitas</h2>
        ...
        <div class="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center">
            <form method="GET" action="{{ route('dashboard.manager.facilities') }}"
                class="flex w-full gap-2 flex-col sm:flex-row">
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama fasilitas"
            ...
            <button type="button" id="open-create-facility"
                class="w-full sm:w-auto rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-700">Create
                Fasilitas</button>
        </div>

        <div id="create-facility-form" class="mt-4 hidden rounded-xl border border-slate-200 bg-slate-50 p-4">
            <h3 class="text-sm font-semibold text-slate-900">Tambah Fasilitas</h3>
            <form method="POST" enctype="multipart/form-data" action="{{ route('dashboard.manager.facilities.store') }}"
                ...
                <div class="flex items-center gap-2 pt-6">
                    <input type="checkbox" id="is_active_create_facility" name="is_active" value="1" checked>
                    <label for="is_active_create_facility" class="text-xs text-slate-700">Tampilkan di landing page</label>
                </div>
                <div class="sm:col-span-2 flex flex-col sm:flex-row sm:justify-end gap-2">
                    <button type="button" id="cancel-create-facility"
        ...
        <div class="mt-5 space-y-3 max-w-7xl mx-auto">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="space-y-1">
                            ...
                            <p
                                ...
                                {{ $facility->is_active ? 'Aktif ditampilkan' : 'Tidak ditampilkan' }}
                            </p>
                        </div>
                        <div class="flex w-full flex-col gap-2 sm:w-32">
                            <button type="button" data-toggle="edit-{{ $facility->id }}"
                                class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100">Edit</button>
                            <form method="POST"
                                action="{{ route('dashboard.manager.facilities.delete', ['facility' => $facility->id]) }}">
                                @csrf
                                <button type="button" data-delete-name="{{ $facility->name }}"
                                    class="delete-facility-btn w-full rounded-lg bg-rose-600 px-3 py-2 text-xs font-semibold text-white hover:bg-rose-700">Hapus</button>
                            </form>
                        </div>
                    </div>

                    <div id="edit-{{ $facility->id }}" class="mt-4 hidden rounded-lg border border-slate-200 bg-white p-4">
                        <h4 class="mb-3 text-sm font-semibold text-slate-900">Edit Fasilitas</h4>
                        <form method="POST" enctype="multipart/form-data"
                            action="{{ route('dashboard.manager.facilities.edit', ['facility' => $facility->id]) }}"
                            ...
                            <div class="flex items-center gap-2 pt-6">
                                <input type="checkbox" id="is_active_facility_{{ $facility->id }}" name="is_active"
                                    value="1" {{ $facility->is_active ? 'checked' : '' }}>
                                <label for="is_active_facility_{{ $facility->id }}"
                                    class="text-xs text-slate-700">Tampilkan di landing page</label>
                            </div>
            ...
            @empty
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Fasilitas tidak
                    ditemukan.</div>
            @endforelse
        </div>
    </section>

    <div id="delete-modal-facility" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" id="modal-backdrop-facility">
            </div>
            ...
            <div
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg font-semibold leading-6 text-slate-900" id="modal-title-facility">
                            Hapus Fasilitas
                        </h3>
                        <div class="mt-2">
                            <p class="text-sm text-slate-500" id="modal-message-facility">
                ...
                <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse sm:gap-3">
                    <button type="button" id="confirm-delete-btn-facility"
                    ...
                    <button type="button" id="cancel-delete-btn-facility"
    ...
    <script>
        (function() {
            const openButton = document.getElementById('open-create-facility');
            const cancelButton = document.getElementById('cancel-create-facility');
            const createForm = document.getElementById('create-facility-form');
            const modal = document.getElementById('delete-modal-facility');
            const backdrop = document.getElementById('modal-backdrop-facility');
            const confirmBtn = document.getElementById('confirm-delete-btn-facility');
            const cancelDeleteBtn = document.getElementById('cancel-delete-btn-facility');
            const modalTitle = document.getElementById('modal-title-facility');
            const modalMessage = document.getElementById('modal-message-facility');
            ...
            function showModal(name, form) {
                ...
                modalTitle.textContent = `Hapus Fasilitas: ${name}`;
                modalMessage.textContent =
                    `Apakah Anda yakin ingin menghapus fasilitas "${name}"? Tindakan ini tidak dapat dibatalkan.`;
                deleteForm = form;
            ...
            document.querySelectorAll('.delete-facility-btn').forEach(function(button) {
                button.addEventListener('click', function() {
