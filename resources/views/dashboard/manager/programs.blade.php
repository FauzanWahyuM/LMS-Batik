            ...
            <form method="POST" action="{{ route('dashboard.manager.programs.store') }}"
                ...
                <div>
                    ...
                    <input type="text" name="duration" required
                        class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                        placeholder="Contoh: 2 Januari 2026">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">Biaya Program (Rupiah)</label>
                    <input type="number" min="0" step="1000" name="cost" required
                ...
                <article class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="space-y-1">
                            ...
                            <p class="text-xs text-slate-600">Durasi: {{ $program['duration'] }}</p>
                            <p class="text-xs text-slate-600">Biaya: Rp
                                {{ number_format((int) $program['cost'], 0, ',', '.') }}</p>
                    ...
                    <div id="edit-{{ $program['id'] }}"
                        ...
                        <form method="POST"
                            ...
                            <div>
                                <label class="mb-1 block text-xs font-semibold text-slate-600">Biaya Program
                                    (Rupiah)
                                </label>
                                <input type="number" min="0" step="1000" name="cost"
                                    value="{{ (int) $program['cost'] }}" required
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
                            </div>
                            <div class="sm:col-span-2">
                                <label class="mb-1 block text-xs font-semibold text-slate-600">Deskripsi Program</label>
