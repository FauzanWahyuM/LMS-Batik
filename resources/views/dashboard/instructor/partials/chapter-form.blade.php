<details class="rounded-lg border border-slate-300 bg-white" data-chapter-index="{{ $index }}" data-chapter-card>
    ...
    <div class="space-y-4 border-t border-slate-200 p-4">
        ...
        <div>
            ...
            <input type="hidden" name="chapters[{{ $index }}][content]"
                data-rich-editor-target="chapter-content-{{ $index }}"
                value="{{ old('chapters.' . $index . '.content', $chapter['content'] ?? '') }}">
            <div data-rich-editor="chapter-content-{{ $index }}"
                class="min-h-[180px] rounded-lg border border-slate-300 bg-white"></div>
            <p class="mt-1 text-xs text-slate-500">Gunakan editor untuk format teks dan sisipkan gambar. Gambar
                tersimpan langsung di database.</p>
        </div>
