                    ...
                    <div
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-3 flex items-center gap-2">
                                @if ($submission->isGraded())
                                    @if ($submission->score !== null && $submission->score < 65)
                        ...
                        <div class="flex w-full flex-col gap-2 lg:w-auto">
                            <a href="{{ route('dashboard.instructor.assessments.detail', ['submission' => $submission->id]) }}"
                                class="rounded-lg {{ $submission->isGraded() ? 'border border-slate-300 text-slate-700 hover:bg-slate-50' : 'bg-slate-700 text-white hover:bg-slate-800' }} px-4 py-2.5 text-center text-sm font-semibold transition">
                                {{ $submission->isGraded() ? 'Edit' : 'Nilai' }}
                            </a>
                        </div>
                    </div>
                @empty
